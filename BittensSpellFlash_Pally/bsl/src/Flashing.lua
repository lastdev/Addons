...
if u.SkipOrUpgrade(c, "Flashing", tonumber("20150202022148") or time()) then
   return
end
...
function c.GetDelay(spell)
   ...
   local holdFor = spell.HoldFor and spell:HoldFor() or 0
...
--c.Debug("DelayPriorityFlash", name, delay, modDelay)
         if delay then
            if spell.IsMinDelayDefinition then
               ...
               delay = max(delay, minDelay or spell.MaxWait)

...
--Nightmare Breath triggers 7.2 iCD (formerly 9.7) (except on roots it's 3)
...
--Shattering Earth triggers 7.2 ICD (except on roots it's 3)
--Shattering Roots itself is unaffected by ICDs of other spells
local function updateAllTimers(self, ICD, special)
	...
	--Roots is 2.4 IDC except when it's crushing grip then it's doubled to 4.8
	if special or (ICD == 6 and (timerRootsCD:GetRemaining(self.vb.rootsCount+1) < 4.8)) then
		local elapsed, total = timerRootsCD:GetTime(self.vb.rootsCount+1)
		local extend = special and 2.4 or 4.8 - (total-elapsed)
	...
	if timerUprootCD:GetRemaining(self.vb.uprootCount+1) < (special and 3 or ICD) then
		local elapsed, total = timerUprootCD:GetTime(self.vb.uprootCount+1)
		local extend = (special and 3 or ICD) - (total-elapsed)

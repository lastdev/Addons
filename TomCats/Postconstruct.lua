...
do
    ...
    function CHAT_MSG_CHANNEL_NOTICE(_, _, text, _, _, _, _, _, _, _, channelBaseName)
        if (text == "WRONG_PASSWORD") then
            if (channelBaseName == GetAddonChannelName()) then
                addonChannelAltNum = (addonChannelAltNum or 0) + 1;
    ...
    function CHAT_MSG_CHANNEL_NOTICE_USER(_, _, text, _, _, _, _, _, _, _, channelName, _, _, ownerGUID)
        if (joined) then
            local playerGUID = UnitGUID("player")
            if (text == "OWNER_CHANGED" and channelName == GetAddonChannelName() and playerGUID == ownerGUID) then

...
L:SetMiscLocalization({
	BWIconMsg			 =	"DBM передал маркировку иконок в рейде игроку-помощнику BigWigs, чтобы избежать конфликтов с иконками. Убедитесь, что у него включена маркировка, или разжалуйте его, чтобы включить маркировку DBM, или включите опцию переопределения в параметрах Вектиса."
})
...
L:SetOptionLocalization({
	timerCallofCrawgCD		= "Отсчет времени до начала формирования Крогов",
	timerCallofHexerCD 		= "Отсчет времени до начала формирования Ведьмаков",
...
L:SetOptionLocalization({
	...
	timerStageThreeBerserk	= "Отсчет времени до того момента, когда адды на 3-й фазе впадут в ярость"
})
...
L:SetOptionLocalization({
	...
	InterruptBehavior	= "Установить режим прерываний для $spell:310788 (перезапишет все остальные настройки, если Вы лидер рейда)",
...
L:SetOptionLocalization({
	InterruptBehavior	= "Установить режим прерываний для $spell:224088 (перезапишет все остальные настройки, если Вы лидер рейда)",
	Four				= "Ротация из 4 человек ",
	Five				= "Ротация из 5 человек ",--По умолчанию
	Six					= "Ротация из 6 человек ",
	NoReset				= "бесконечное увеличение ",
	ArrowOnGlare		= "Показывать стрелку влево/вправо для направления $spell:317874",

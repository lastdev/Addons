...
if ns.author then
	ns.wmp, ns.wmpStr = 4, "4"
else
	ns.wmp, ns.wmpStr = 2, "2"
end
...
local function FormattedXY( rawX, rawY )
	if ns.author then
		return format( "%." ..ns.wmpStr .."f", round( rawX, ns.wmpStr ) ),
				format( "%." ..ns.wmpStr .."f", round( rawY, ns.wmpStr ) )
	else
...
local function OnEventHandler( self, event, args )
	...
	if ( event == "VARIABLES_LOADED" ) then
		...
		elseif locale == "esES" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-esES" )
		elseif locale == "esMX" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-esMX" )
		elseif locale == "frFR" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-frFR" )
		elseif locale == "itIT" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-itIT" )
		elseif locale == "koKR" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-koKR" )
		elseif locale == "ptBR" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-ptBR" )
		elseif locale == "ptPT" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-ptPT" )
		elseif locale == "ruRU" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-ruRU" )
		elseif locale == "zhCN" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-zhCN" )
		elseif locale == "zhTW" then
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title-zhTW" )
		else
			addonTitle = C_AddOns.GetAddOnMetadata( "XandY", "Title" )
		end
...
local function OnUpdate()
	...
	if WorldMapFrame:IsVisible() == true then
		...
		world.xPlayer, world.yPlayer = round( ( world.xPlayer or 0 ) * 100, ns.wmp ), round( ( world.yPlayer or 0 ) * 100, ns.wmp )
		world.xCursor, world.yCursor = WorldMapFrame:GetNormalizedCursorPosition()	
		world.xCursor, world.yCursor = round( world.xCursor * 100, ns.wmpStr ), round( world.yCursor * 100, ns.wmpStr )
		
		world.playerCursor, world.spaces = "", ""
		if ( world.xPlayer ~= 0 ) or ( world.yPlayer ~= 0 ) then
		
			world.playerCursor = L[ "Player" ] .." @ " ..ns.colour.malachite ..format( "%." ..ns.wmpStr .."f", world.xPlayer )
									.."\124r:" ..ns.colour.malachite ..format( "%." ..ns.wmpStr .."f", world.yPlayer ) .."\124r"
			world.spaces = " "
		end		
		...
		if buildVersion < 6 then
			XandYCoords:SetText( world.playerCursor )
			if buildVersion >= 5 then
				world.offset = ( ( WorldMapFrame:GetHeight() / 2 ) - 18 ) * -1
			else
				world.offset = ( ( WorldMapFrame:GetHeight() / 2 ) - 18 ) * -1
			end
			if buildVersion < 3 then
				...
				XandYCoords:SetPoint( "CENTER", WorldMapFrame, "CENTER", -72, world.offset ) -- Used to be -216 offset always
			end
		elseif buildVersion >= 6 then

...
do
  ...
  function SI:BugReport(msg)
    local now = GetTime()
    if bugReported[msg] and now < bugReported[msg] + 60 then
      return
    end
    bugReported[msg] = now
    SI:ChatMsg(msg)

    if bugReported["url"] and now < bugReported["url"] + 5 then
      return
    end
    bugReported["url"] = now
...
function SI:ClassColorString(toon, str)
  ...
  return format("|c%02x%02x%02x%02x%s%s", floor(a * 255), floor(r * 255), floor(g * 255), floor(b * 255), str, FONT_COLOR_CODE_CLOSE)
end

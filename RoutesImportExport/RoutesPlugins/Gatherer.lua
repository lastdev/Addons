-- modified (fixed) plugin from Routes addon

local Routes = LibStub("AceAddon-3.0"):GetAddon("Routes", 1)
if not Routes then return end

function Routes.InitFixedGathererPlugin()
	...
	if not enabled or (reason ~= nil and reason ~= "DEMAND_LOADED" and strlen(reason) > 0) then
		loaded = false
		return
	end

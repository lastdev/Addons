...
local function _SetQuantity(frame, quantity)
	frame.Quantity:SetText(quantity)
	frame.Quantity:Show()
end

local function _SetItemLevel(frame, itemID)
	local text = ""
...
local function _SetFollowerXP(frame, xp)
	if xp >= 10000 then
...
local function _SetMoney(frame, money)
...
local function _SetReward(frame, reward)
	frame.Quantity:Hide()
	frame.itemID = nil
	frame.title = nil
	frame.tooltip = nil
...
addon:RegisterClassExtensions("AltoGarrisonMissionReward", {
	SetQuantity = _SetQuantity,
	SetItemLevel = _SetItemLevel,
	SetFollowerXP = _SetFollowerXP,
	SetMoney = _SetMoney,
	SetReward = _SetReward,
})

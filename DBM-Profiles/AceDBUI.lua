...
local function createButton(parent)
	local button = CreateFrame("Button", getWidgetName(), parent, "UIMenuButtonStretchTemplate")
	return button
end
...
function AceDBUI:CreateUI(name, db, frame)
	...
	newProfile:SetSize(160, 20)
	newProfile:SetPoint("TOP", reset, "BOTTOM", 0, -8)
	...
	choose:SetPoint("TOP", newProfile, "BOTTOM", 0, -16)
	...
	copy:SetWidth(88)
	copy:SetPoint("TOPLEFT", choose, "BOTTOMLEFT", 16, -4)
	copy:SetText("Copy from")
	copy.menu.func = copyProfileOnClick
	copy.menu.nocurrent = true
	objects.copy = copy.menu

	local delete = createMenuButton(frame)
	delete:SetWidth(88)
	delete:SetPoint("TOPRIGHT", choose, "BOTTOMRIGHT", -16, -4)
	delete:SetText("Delete")
	delete.menu.func = deleteProfileOnClick
	delete.menu.nocurrent = true
	objects.delete = delete.menu
	
	do
		local dualProfile = createDropdown(frame)
		dualProfile:SetPoint("TOP", copy, "BOTTOM", 0, -32)

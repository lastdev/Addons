...
-- spells
local SURGING_MIST, CHI_HEAL, ENVELOPING_MIST, SOOTHING_MIST, RENEWING_MIST, UPLIFT, REVIVAL, LIFE_COCOON, CROWD_CONTROL, MANATEA, MANATEA_BUFF, THUNDERFOCUSTEA, CHIBREW, DETOX, TIGERS_LUST, VITAL_MISTS_BUFF, RESUSCITATE;
local SURGING_MIST_ID, CHI_HEAL_ID, ENVELOPING_MIST_ID, SOOTHING_MIST_ID, RENEWING_MIST_ID, UPLIFT_ID, REVIVAL_ID, LIFE_COCOON_ID, CROWD_CONTROL_ID, MANATEA_ID, MANATEA_BUFF_ID, THUNDERFOCUSTEA_ID, CHIBREW_ID, DETOX_ID, TIGERS_LUST_ID, VITAL_MISTS_BUFF_ID, RESUSCITATE_ID;
...
function MistWeaver_SlashCommandHandler(msg)
    ...
    elseif (string.find(msg, "sorttype") == 1) then
        ...
        MistWeaver_RebindUnitFrames();
    elseif (string.find(msg, "stat") == 1) then
        local arg = string.sub(msg, 6);
        MistWeaver_Statistics_Show(arg);
    end
...
function MistWeaver_Help()
    ...
    mw_info("|c333399ff      /mw stat <channel> <name>|r "..STATISTICS);
...
function MistWeaver_UpdateActiveState()
    ...
    local shapeshiftOk = MistWeaverData.SHOW_IN_ALL_STANCES == 1 or GetShapeshiftForm() == serpentStance;
    
    local activeSpecGroup = GetActiveSpecGroup();
    local specialization = GetSpecialization(_, _, activeSpecGroup);
    local specializationOk = specialization == 2;
    local groupOk = MistWeaverData.SHOW_SOLO == 1 or GetNumGroupMembers() > 0;

    if (unitClass == "MONK" and shapeshiftOk and specializationOk and groupOk and MistWeaverData.ACTIVE == 1) then
...
function MistWeaver_LoadSpells()
    ...
    CROWD_CONTROL_ID = nil;
    CROWD_CONTROL = nil;
    if (GetSpellInfo(ringOfPeace)) then
        CROWD_CONTROL_ID = 116844;
        CROWD_CONTROL = ringOfPeace;
    elseif (GetSpellInfo(chargingOxWave)) then
        CROWD_CONTROL_ID = 119392;
        CROWD_CONTROL = chargingOxWave;
    elseif (GetSpellInfo(legSweep)) then
        CROWD_CONTROL_ID = 119381;
        CROWD_CONTROL = legSweep;
    end
    ...
    MANATEA_BUFF = GetSpellInfo(MANATEA_ID);
...
function MistWeaver_OnButtonUpdate(self, elapsed)
    ...
    elseif (nomana) then
        texture:SetVertexColor(0.5, 0.5, 1.0);
    else
...
function MistWeaver_ChiBrewOnUpdate(self, elapsed)
    ...
    if (start > 0 and duration > 0) then
        MwPowerFrameChi.text:SetTextColor(1.0, 1.0, 1.0); -- has cooldown -> not usable
    elseif (usable) then
        MwPowerFrameChi.text:SetTextColor(0.0, 1.0, 0.0); -- usable
    elseif (nomana) then
        MwPowerFrameChi.text:SetTextColor(1.0, 1.0, 1.0); -- not usable
    else
        MwPowerFrameChi.text:SetTextColor(1.0, 1.0, 1.0); -- not usable
    end
end
...
function MistWeaver_Init()
    ...
    chiBar:SetPoint("BOTTOMRIGHT", powerFrame, "BOTTOMRIGHT", -4, 16);
    chiBar:SetStatusBarColor(0.85, 0.85, 0.85);
    chiBar:SetFrameStrata("LOW");
    
    -- chi brew
    local chiBrew = MistWeaver_CreateFrame("Button", "MwUnitFrameChiBrew", chiBar, "SecureActionButtonTemplate");
    chiBrew:SetFrameStrata("HIGH");
    chiBrew:ClearAllPoints();
    chiBrew:SetPoint("TOPLEFT", chiBar, "TOPLEFT", 0, 0);
    chiBrew:SetPoint("BOTTOMRIGHT", chiBar, "BOTTOMRIGHT", 0, 0);
    MistWeaver_RegisterButton(chiBrew);
    chiBrew:SetAttribute("type", "spell");
    chiBrew:SetAttribute("spell", CHIBREW);
    chiBrew:SetAttribute("spellId", CHIBREW_ID);
    ...
    -- AE
    local revivalCast = MistWeaver_CreateActionButton("MwRevivalCastFrame", 64);
    revivalCast:SetPoint("TOPRIGHT", powerFrame, "TOPLEFT", 0, 0);
    
    -- left/right
    local envelopingMistCast = MistWeaver_CreateActionButton("MwEnvelopingMistCastFrame", 33, KEY_BUTTON1);
    envelopingMistCast:SetPoint("TOPLEFT", revivalCast, "BOTTOMLEFT", 0, vert_space);
    
    local soothingMistCast = MistWeaver_CreateActionButton("MwSoothingMistCastFrame", 33, KEY_BUTTON2);
    soothingMistCast:SetPoint("TOPLEFT", envelopingMistCast, "TOPRIGHT", -2, 0);
   
    -- shift left/right
    local surgingMistCast = MistWeaver_CreateActionButton("MwSurgingMistCastFrame", 33, "["..SHIFT_KEY.."] + "..KEY_BUTTON1);
    surgingMistCast:SetPoint("TOPLEFT", envelopingMistCast, "BOTTOMLEFT", 0, 2);   
    
    local chiHealCast = MistWeaver_CreateActionButton("MwChiHealCastFrame", 33, "["..SHIFT_KEY.."] + "..KEY_BUTTON2);
    chiHealCast:SetPoint("TOPLEFT", surgingMistCast, "TOPRIGHT", -2, 0);
    
    -- middle
    local lifeCocoonCast = MistWeaver_CreateActionButton("MwLifeCocoonCastFrame", 64, KEY_BUTTON3);
    lifeCocoonCast:SetPoint("TOPLEFT", surgingMistCast, "BOTTOMLEFT", 0, vert_space);
    
    local crowdControlCast = MistWeaver_CreateActionButton("MwCrowdControlCastFrame", 64, "["..SHIFT_KEY.."] + "..KEY_BUTTON3);
    crowdControlCast:SetPoint("TOPLEFT", lifeCocoonCast, "BOTTOMLEFT", 0, 2);
    
    -- left/right
    local upliftCast = MistWeaver_CreateActionButton("MwUpliftCastFrame", 33, KEY_BUTTON1);
    upliftCast:SetPoint("TOPLEFT", crowdControlCast, "BOTTOMLEFT", 0, vert_space);
    
    local renewingMistCast = MistWeaver_CreateActionButton("MwRenewingMistCastFrame", 33, KEY_BUTTON2);
    renewingMistCast:SetPoint("TOPLEFT", upliftCast, "TOPRIGHT", -2, 0);
    
    -- mana tea
    local manaTea = MistWeaver_CreateActionButton("MwManaTeaFrame", 42);
    manaTea:SetPoint("TOPLEFT", powerFrame, "TOPRIGHT", 1, 0);
    
    -- thunder focus tea
    local thunderFocusTea = MistWeaver_CreateActionButton("MwThunderFocusTeaFrame", 42);
    thunderFocusTea:SetPoint("TOPLEFT", manaTea, "TOPRIGHT", -2, 0);
...
function MistWeaver_RebindButtonSpells()
    ...
    MistWeaver_RebindButtonSpell("MwChiHealCastFrame", CHI_HEAL, CHI_HEAL_ID);
    MistWeaver_RebindButtonSpell("MwLifeCocoonCastFrame", LIFE_COCOON, LIFE_COCOON_ID);
    MistWeaver_RebindButtonSpell("MwCrowdControlCastFrame", CROWD_CONTROL, CROWD_CONTROL_ID);
...
function MistWeaver_InitUnitFrame(unitFrame)
    ...
    if (MistWeaverData.SHOW_RAID_EXTENSION == 1) then    
    	offsetStats = MistWeaverData.RAID_EXTENSION_WIDTH;
    end
    ...
    aggroBar:SetPoint("TOPLEFT", healthBar, "TOPLEFT", 0, 1);
    aggroBar:SetPoint("BOTTOMRIGHT", healthBar, "TOPRIGHT", 0, -2);
    aggroBar:SetStatusBarColor(1.0, 0.0, 0.0);
    aggroBar:SetFrameStrata("LOW");
...
function MistWeaver_ResizeRaidExtensionForFrame(unitFrame)	
    ...
    if (unitFrame) then
		...
		if (MistWeaverData.SHOW_RAID_EXTENSION == 1) then 
			...
			statsFrame = _G[framename.."Stats"]; 
			statsFrame:SetPoint("TOPLEFT", 0, 0);
		end
    end
end
...
function MistWeaver_InfoButtonOnEnter(frame)
    ...
    CleanUI_ShowHealingTooltip(frame);
end
...
function MistWeaver_InitInfoMenu(self)    
    ...
    UIMenu_AddButton(self, MW_SHOW_DEBUFFS, nil, MwConfig_ToggleShowDebuffs);  
...
function MistWeaver_UpdateUnitFrameSize()
    ...
    MwPowerFrame:SetWidth(max(MistWeaverData.UNITWIDTH, 125));
...
function MistWeaver_RebindSpells(frame)
    ...
    button1:SetAttribute("shift-spell2", CHI_HEAL);
    
    button1:SetAttribute("shift-type3", "spell");
    button1:SetAttribute("shift-spell3", CROWD_CONTROL);
    
    button1:SetAttribute("alt-type1", "spell");
    button1:SetAttribute("alt-spell1", DETOX);
    ...
    if (TIGERS_LUST) then
        ...
        button1:SetAttribute("alt-spell2", TIGERS_LUST);
    end
...
function MistWeaver_UpdateRaidIcon(frame)
    ...
    if (not frame.unit or MistWeaverData.SHOW_RAID_ICON ~= 1) then
        raidIcon:Hide();
        return;
    end
...
function MistWeaver_UpdatePlayerBuffs()
    --local VITAL_MISTS_BUFF_ID = 118674;
    --local MANATEA_BUFF_ID = 115867;
    local index = 1;
    ...
    -- mana tea buff
    name, rank, icon, count = UnitBuff("player", MANATEA_BUFF);
    if (name) then
...
function MistWeaver_ShowThunderFocusTeaOverlayGlow()
    ...
    MistWeaver_ShowOverlayGlow(_G["MwUpliftCastFrame"]);
end
...
function MistWeaver_HideThunderFocusTeaOverlayGlow() 
    ...
    MistWeaver_HideOverlayGlow(_G["MwUpliftCastFrame"]);
end
...
function MistWeaver_UpdateUnitHealth(frame, unit)
    ...
    if (MistWeaverData.SHOW_PVP_ICON == 1 and UnitIsPVP(unit)) then
...
function MistWeaver_UpdateRaidDebuffColor(frame, unit, debuffColor, expirationTime)
    if (MistWeaverData.SHOW_RAID_EXTENSION ~= 1 or MistWeaverData.RAID_SHOW_DEBUFFS ~= 1) then  
	    return;
	end
    ...
    if (UnitExists(unit) and (not UnitIsEnemy("player", unit)) and debuffColor and usable) then   --  and duration == 0
		...
		CooldownFrame_SetTimer(_G[frame:GetName().."DetoxCooldown"], start, duration, enabled);
    else
    	detoxFrame:SetAlpha(0);
    	detoxExpirationTime:SetText("");
    end
...
function MistWeaver_RebindRaidData(frame, unit)
    if (MistWeaverData.SHOW_RAID_EXTENSION ~= 1) then  
		...
		frame.art.raidIcon:SetPoint("TOPLEFT", frame.art, "TOPLEFT", 2, -2);	
		frame.art.raidIcon:SetSize(12, 12);

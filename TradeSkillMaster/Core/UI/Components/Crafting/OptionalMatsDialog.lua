...
function OptionalMatsDialog.__private:_GetOptionalMatsViewContentFrame(viewContainer, path)
	if path == "list" then
		...
		return UIElements.New("Frame", "frame")
			...
			:AddChild(UIElements.New("ScrollFrame", "list")
				:SetPadding(0, 8, 0, 8)
				:SetMargin(0, 0, 0, 8)
	...
	elseif path == "selection" then
		return UIElements.New("Frame", "frame")
			...
			:AddChild(UIElements.New("Frame", "container")
				...
				:SetRoundedBackgroundColor("PRIMARY_BG_ALT")
				:AddChild(UIElements.New("ScrollFrame", "items")
...
function OptionalMatsDialog.__private:_HandleOptionalMatRemoveClick(button)
	...
	local tooltip = private.GetResultInfo(self._recipeString)
	...
	titleFrame:GetElement("name")
		:SetText(Profession.GetMatSlotText(CraftString.FromRecipeString(self._recipeString), matString))
	titleFrame:GetElement("removeBtn")
		:Hide()
	titleFrame:Draw()
	titleFrame:GetElement("__parent.__parent.__parent.item.icon")
		:SetTooltip(tooltip)
		:Draw()
end

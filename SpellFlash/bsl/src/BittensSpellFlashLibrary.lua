...
if u.SkipOrUpgrade(c, "MainFile", 2) then
	return
end
...
--	local id = c.GetID(name)
--	return s.CurrentSpell(id) 
--		or s.Casting(id, "player") 
--		or s.Channeling(id, "player")
--end
...
function c.AssociateTravelTimes(estimate, ...)
	...
	for i = 1, select("#", ...) do
		c.A.TravelTimes[select(i, ...)] = info
	end
end

function c.EstimateTravelTime(name)
	if c.A.TravelTimes then
		local info = c.A.TravelTimes[name]
...
function c.PriorityFlash(...)
	...
	if c.Debugging then
		...
		if rotation.ExtraDebugInfo then
			c.Debug("Flashing", rotation.ExtraDebugInfo(), flashed)
		else
			c.Debug("Flashing", flashed)
		end
	end
	return flashed
end

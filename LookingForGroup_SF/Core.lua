...
--		{"CHAT_MSG_GUILD"}
	...
	while true do
		...
		if profile.spam_filter_disable then
			break
		end
		...
		if tag < 5 then
			if LookingForGroup.realm_filter(player) then
				whisper = 0
				break
			end
		...
		if tag < 5 then
			...
			if profile.spam_filter_spaces then
				local num_spaces
			...
			if filters then
				...
				if not profile.spam_filter_player_name then
					player = player:lower()
					for i=1,#filters do
...
function SF:OnInitialize()
	local db = LookingForGroup.db
	if db.profile.disable_gmotd then

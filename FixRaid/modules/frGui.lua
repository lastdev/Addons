--- Define a GUI for the /fr (/fixraid) console command.
...
local function getCommand(cmd, aliases, isShiftClick)
  ...
  elseif aliases and #aliases > 0 then
    local text = format("%s %s %s", H("/fr "..cmd), HD(L["word.or"]), H("/fr "..aliases[1]))
    if #aliases > 1 then
...
function M:Open()
  ...
  R.label:SetText(format(L["gui.fixRaid.intro"], H("/fr"), H("/fixraid")))
  R.label:SetFullWidth(true)
  c:AddChild(R.label)

  local header = AceGUI:Create("Heading")
  header:SetText(format(L["gui.header.buttons"], H("/fr")))

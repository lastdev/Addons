...
function M:CHAT_MSG_ADDON(event, prefix, message, channel, sender)
  ...
  if A.DEBUG >= 1 then A.console:Debugf(M, "CHAT_MSG_ADDON prefix=%s channel=%s sender=%s message=%s", prefix, channel, (sender == UnitName("player")) and sender or H(sender), message) end
...
function M:Broadcast(message)
  if IsInGroup() then
    C_ChatInfo.SendAddonMessage(PREFIX, message, A.util:GetGroupChannel())
  end
end

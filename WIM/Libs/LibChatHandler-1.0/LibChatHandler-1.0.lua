...
Date: $Date: 2012-01-25 13:52:29 -0600 (Wed, 25 Jan 2012) $
...
local MAJOR, MINOR = "LibChatHandler-1.0", tonumber(("$Revision: 23 $"):match("(%d+)"));
...
if(prevLib and prevLib:GetMinor() < MINOR) then
	...
	if (prevLib:GetMinor() == 15) then
		 ChatFrame_OnEvent_orig = ChatFrame_OnEvent;
	end;
...
local fcfm = FloatingChatFrameManager or nil;
		...
		if(e.flag_sent_to_delegates and #e.suspendedBy == 0) then
			if(not e.flag_block) then
				...
				if(not e.flag_blocked_from_chatFrame) then
					...
					local chatEvent = e:GetEvent();

					if (fcfm ~= nil) then
						local chatType = strsub(chatEvent, 10);
						local chatGroup = Chat_GetChatCategory(chatType);
						if (chatGroup == "BN_WHISPER" or chatGroup == "BN_CONVERSATION" or chatGroup == "WHISPER") then
							FloatingChatFrameManager_OnEvent(fcfm, chatEvent, e:GetArgs())
						end;
					end;
					...
					for j=1,#e.frames do
						...
						if(fName and str_find(fName, "^ChatFrame%d+$")) then
							...
							ChatFrame_OnEvent(e.frames[j], chatEvent, e:GetArgs());
						end
					end
				end
			else
...
local function RegisterChatEvent(self, chatEvent, priority)
	...
	if(isChatEvent(chatEvent)) then
		...
		if (fcfm ~= nil) then
			...
			local chatGroup = Chat_GetChatCategory(chatType);
			if (chatGroup == "BN_WHISPER" or chatGroup == "BN_CONVERSATION" or chatGroup == "WHISPER") then
				fcfm:UnregisterEvent(chatEvent);
			end;
		end;

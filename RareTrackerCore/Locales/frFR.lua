...
L["<RT> Resetting current rare timers and requesting up-to-date data."] = "<RTC> Remise-à-zéro des timers actuels et requête de données à jour."
L["<RT> Please target a non-player entity prior to resetting, such that the addon can determine the current shard id."] = "<RTC> Veuillez sélectionner une entité non joueuse avant la remise-à-zéro, afin de permettre à l'addon de déterminer le shard id actuel."
L["<RT> The reset button is on cooldown. Please note that a reset is not needed to receive new timers. If it is your intention to reset the data, please do a /reload and click the reset button again."] = "<RTC> Le bouton de remise-à-zéro est en recharge. Il est à noté qu'un reset n'est pas nécessaire pour recevoir les nouveaux timers. Si vous voulez effectivement réinitialiser les dnnées, veuillez faire un /reload puis cliquer à nouveau sur le bouton de reset."
-- L["<RT> Moving to shard "] = ""

-- Chat messages.
L["<RT> %s has died"] = "<RTC> %s est mort."
L["<RT> %s (%s%%)"] = "<RTC> %s (%s%%)"
L["<RT> %s (%s%%) seen at %s"] = "<RTC> %s (%s%%) vu à %s"
L["<RT> %s was last seen ~%s minutes ago"] = "<RTC> %s vu pour la dernière fois il y a ~%s minutes"
L["<RT> %s seen alive, vignette at %s"] = "<RTC> %s vu en vie à la position %s"
L["<RT> %s seen alive (combat log)"] = "<RTC> %s vu en vie (log de combat)"
...
L["Left-click: hide/show RT"] = "Clique-gauche : montrer ou masquer RTC"

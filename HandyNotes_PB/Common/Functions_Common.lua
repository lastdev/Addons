...
local ceil, fastRandom, find, floor, gsub, ipairs, next = ceil, fastRandom, string.find, floor, string.gsub, ipairs, next
...
-- My AddOn system throttles the frequency of th HandyNotes update cycling. This is to ensure that there's minimal impact upon the
-- player's FPS, especially on potato PC's. A consequence of this is that the Minimap will not instantly be updated when a player
-- action, such as completing a quest or achievement, would result in a pin disappearing.

-- The throttling also gets around a rare problem that HN has when it wants to update but a cinematic is playing or there's
-- exceptional lag. The arbitrary 60s / 10s in the ns.delay timer in practice is quite sufficient.
...
local function NewClusterPins()
	...
	for i = 1, #ns.clusterNames do -- ns.clusterNames is setup in Data_xxx
		ns.cluster[ ns.clusterNames[ i ] ] = UniqueTextureIndex()
	end	
end
		...
		if pin then
			if ns.PassGeneralChecks( pin ) and
				...
				elseif pin.cluster then
					hash = ns.cluster[ pin.cluster ]
					if pin.cluster == "flavour" then
						return coord, nil, ( ns.textures[ hash ] or nil ),
								ScalePin( hash ) * 2 * ( ( pin.scaling == nil ) and 1 or pin.scaling ), _G[ ns.db ].IconAlpha
					else
						return coord, nil, ( ns.textures[ hash ] or nil ),
								ScalePin( hash ) * 1.5 * ( ( pin.scaling == nil ) and 1 or pin.scaling ), _G[ ns.db ].IconAlpha
					end
...
local function CorrectMapPhase( old, mapID )
	...
	-- Tirisfal, Arathi and Darkshore have Zidormi quirks. Regardless, this routine always works.
	-- In Darkshore I think I was dealing with several phases (those quests leading up to the burning). Probably moot now.
	-- Arathi gives two different spellID buffs. Can test with login/out enter/exit the zone but this routine is reliable.
	-- In Tirisfal the player often needs to do Zidormi TWICE to go to the "old times" but the player is always flagged correctly.

...
--* Release. Built upon that used in EAP and APLT and ARH

--== v2.01 - 9th October 2025
--* Improved logic for CompletionFormatAndShow and AddOnEventTooltipLines
--* Bug with character headers not showing completion
--* Better use of ns.aoa table

--== v2.02 - 10th October 2025
--* Removed copy pasted block that belonged in the ARH AddOn's own block
...
local function CharOrAcctName( id )
	...
	return ( ns.aoa[ id ] == nil or ns.version < 100000 ) and ns.name or ns.L[ "Account" ]
end
					...
					if ns.achievementIQ[ v.id ] then
						...
						for i = iStart, iEnd do -- v.iStart, v.iEnd is for splitting massive criteria lists between pins
							...
							elseif cType == 8 then
								...
								if ( cType == 27 ) or ( ns.version < 100000 ) then

	...
	if pin.pets then
		...
		for _, p in ipairs( pin.pets ) do
			if PassAllChecks( p ) then
				...
				if GuideTip( p ) then GameTooltip:AddLine( "\n" ) end
			end
		end
	end
...
local function ShowQuests( pin )
	...
	if PassAllChecks( pin.quests ) == true then
		for i, v in ipairs( ns.questTypes ) do
			for _, q in ipairs( pin.quests ) do
				if PassAllChecks( q ) and ( q.qType == v ) and PassOracleFrenzyheart( q ) then
	...
	local function iterator( t, prev )
		...
		while coord do
			if pin and PassAllChecks( pin ) and 
				...
				if pin.orphan then
					return coord, nil, ns.textures[ ns.db.iconOrphan ],
							ns.db.iconScale * ns.scaling[ ns.db.iconOrphan ], ns.db.iconAlpha
				elseif pin.achieves then
					return coord, nil, ns.textures[ ns.db.iconAchieves ],
							ns.db.iconScale * ns.scaling[ ns.db.iconAchieves ], ns.db.iconAlpha
				...
				elseif pin.badEx then
					return coord, nil, ns.textures[ ns.db.iconBadEx ],
							ns.db.iconScale * ns.scaling[ ns.db.iconBadEx ], ns.db.iconAlpha
				elseif pin.flavour then
					return coord, nil, ns.textures[ ns.db.iconFlavour ],
							ns.db.iconScale * 2 * ns.scaling[ ns.db.iconFlavour ], ns.db.iconAlpha
				elseif pin.pets then
					return coord, nil, ns.textures[ ns.db.iconPets ],
							ns.db.iconScale * ns.scaling[ ns.db.iconPets ], ns.db.iconAlpha
				elseif pin.related then
					return coord, nil, ns.textures[ ns.db.iconRelated ],
							ns.db.iconScale * ns.scaling[ ns.db.iconRelated ], ns.db.iconAlpha
				elseif pin.utgarde then
					return coord, nil, ns.textures[ ns.db.iconUtgarde ],
							ns.db.iconScale * ns.scaling[ ns.db.iconUtgarde ], ns.db.iconAlpha
				elseif pin.vendor then
					return coord, nil, ns.textures[ ns.db.iconVendor ],
							ns.db.iconScale * ns.scaling[ ns.db.iconVendor ], ns.db.iconAlpha
				end				
			end
...
-- Interface -> Addons -> Handy Notes -> Plugins -> Children's Week options
ns.options = {
	type = "group",
	name = ns.L[ "Children's Week" ],
		...
		notes = {
			...
			args = {
				...
				noteChat = { type = "description", name = ns.L[ "ChatCommands" ]
					..NORMAL_FONT_COLOR_CODE .."/cw" ..HIGHLIGHT_FONT_COLOR_CODE ..", "
					..NORMAL_FONT_COLOR_CODE .."/childrens" ..HIGHLIGHT_FONT_COLOR_CODE ..", "
					..NORMAL_FONT_COLOR_CODE .."/cweek" ..HIGHLIGHT_FONT_COLOR_CODE ..ns.L[ "ShowPanel" ],
					order = 32, },
			},
		},
	},
...
function HandyNotes_ChildrensWeek_OnAddonCompartmentClick( addonName, buttonName )
	...
	LibStub( "AceConfigDialog-3.0" ):SelectGroup( "HandyNotes", "plugins", "ChildrensWeek" )
end
...
function HandyNotes_ChildrensWeek_OnAddonCompartmentEnter( ... )
	...
	GameTooltip:AddLine( ns.colour.prefix ..ns.L[ "Children's Week" ] .."\n\n" )
...
function pluginHandler:OnEnable()
	...
	HandyNotes:RegisterPluginDB( "ChildrensWeek", pluginHandler, ns.options )
	ns.db = LibStub( "AceDB-3.0" ):New( "HandyNotes_ChildrensWeekDB", ns.defaults, "Default" ).profile
	pluginHandler:Refresh()
end
...
function pluginHandler:Refresh()
	if GetTime() > ( ns.delay or 0 ) then
		ns.delay = nil
		self:SendMessage( "HandyNotes_NotifyUpdate", "ChildrensWeek" )
	end
end

LibStub( "AceAddon-3.0" ):NewAddon( pluginHandler, "HandyNotes_ChildrensWeekDB", "AceEvent-3.0" )
...
local function Slash( options )
	...
	if ( ns.version >= 100000 ) then
		print( ns.colour.prefix ..ns.L[ "Children's Week" ] ..": " ..ns.colour.highlight ..ns.L[ "TryMinimapMenu" ] )
	end
end

SlashCmdList[ "ChildrensWeek" ] = function( options ) Slash( options ) end
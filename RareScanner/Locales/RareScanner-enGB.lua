...
if AL then
	...
	AL["LOOT_EXPLORER_FILTER_DESC"] = "In order to use this filter you have to perform a scan with RareScanner's explorer. When activated, you will see only the missing items found via the scan in the loot bar and world map tooltips."
	AL["LOOT_EXPLORER_FILTER_LONG_DESC"] = "In order to use this filter you have to scan with RareScanner's explorer. This process will detect every missing collectible according to the filters applied in the explorer. With this option enabled, you will see only the loot that matches those results. |cffffff00Important!: With this filter enabled you won't be able to use the rest of loot filters.|r"

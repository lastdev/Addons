local __exports = LibStub:NewLibrary("ovale/ui/Version", 90112)
...
__exports.OvaleVersionClass = __class(nil, {
        ...
        self.handleCommReceived = function(prefix, message, channel, sender)
            if prefix == messagePrefix then
                ...
                if ok then
                    ...
                    if msgType == "V" then
                        local msg = self.module:Serialize("VR", "90112")
    ...
    versionCheck = function(self)
        if  not timer then
            wipe(userVersions)
            local message = self.module:Serialize("V", "90112")
            local channel

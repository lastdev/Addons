local __exports = LibStub:NewLibrary("ovale/ui/Version", 90113)
...
__exports.OvaleVersionClass = __class(nil, {
        ...
        self.handleCommReceived = function(prefix, message, channel, sender)
            if prefix == messagePrefix then
                ...
                if ok then
                    ...
                    if msgType == "V" then
                        local msg = self.module:Serialize("VR", "90113")
    ...
    versionCheck = function(self)
        if  not timer then
            wipe(userVersions)
            local message = self.module:Serialize("V", "90113")
            local channel

local __exports = LibStub:NewLibrary("ovale/ioc", 90047)
...
local DebugTools = __enginedebug.DebugTools
...
local Guids = __engineguid.Guids
...
__exports.IoC = __class(nil, {
    constructor = function(self)
        ...
        self.debug = DebugTools(self.ovale, self.options)
        ...
        self.guid = Guids(self.ovale, self.debug, self.condition)
        self.spellBook = OvaleSpellBookClass(self.ovale, self.debug, self.data)
        self.cooldown = OvaleCooldownClass(self.paperDoll, self.data, self.lastSpell, self.ovale, self.debug, self.profiler)
        ...
        self.power = OvalePowerClass(self.debug, self.ovale, self.profiler, self.data, self.baseState, self.spellBook, combat)
        self.state = OvaleStateClass()
        self.aura = OvaleAuraClass(self.state, self.paperDoll, self.baseState, self.data, self.guid, self.lastSpell, self.options, self.debug, self.ovale, self.profiler, self.spellBook, self.power)
        ...
        self.compile = OvaleCompileClass(self.azeriteArmor, self.ast, self.condition, self.cooldown, self.data, self.profiler, self.debug, self.ovale, self.score, self.spellBook, controls, self.scripts)
        ...
        self.spellFlash = OvaleSpellFlashClass(self.options, self.ovale, combat, self.actionBar)

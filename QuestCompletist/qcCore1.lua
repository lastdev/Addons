...
--Bfa		    _, _, _, _, _, isComplete, _, _, _ = GetQuestLogTitle(GetQuestLogIndexByID(questId)) -- TODO: Still same return?
				_, _, _, _, _, isComplete, _, _, _ = C_QuestLog.GetTitleForLogIndex(questLogIndex)(C_QuestLog.GetLogIndexForQuestID(questId)) 
...
local function qcRefreshPins(uimapId, mapLevel)
	...
	if (qcSettings["QC_M_HIDE_INPROGRESS"] == 1) then
		for i = #qcPins, 1, -1 do
			for qcQuestIndex = #qcPins[i][7], 1, -1 do
				local qcQuestID = qcPins[i][7][qcQuestIndex]
				if (C_QuestLog.GetLogIndexForQuestID(qcQuestID) ~= 0) then
					TableRemove(qcPins[i][7], qcQuestIndex)
				end
			end
			if (#qcPins[i][7] == 0) then

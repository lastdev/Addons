...
-- Quest Hide low level
	--	if (qcSettings.QC_L_HIDE_LOWLEVEL == 1) then
		...
		--	end	
		--end	
--      Bitband Code for Hideing Repetable quest	
...
--      Bitband Code for Hideing Weekly quest	
		if (qcSettings.QC_L_HIDE_WORLDQUEST == 1) then
			local questType = 256
			for i = #qcCategoryQuests, 1, -1 do
				if (BitBand(qcCategoryQuests[i][6], questType) == 256) then
					tableRemove(qcCategoryQuests,i)
			end
		end
	end
	...
	for i = 1, 16 do
		...
		if (qcCurrentCategoryQuestCount >= offset) then
			...
			elseif (questType == 128) then
				...
				questRecord.QuestName:SetTextColor(0.0941176470588235, 0.6274509803921569, 0.9411764705882353, 1.0)
			elseif (questType == 256) then
				questRecord.QuestIcon:SetTexCoord(unpack(QC_ICON_COORDS_WEEKLY))-- Todo?? Maybe make own worldquest icon
				questRecord.QuestName:SetTextColor(0.0941176470588235, 0.6274509803921569, 0.9411764705882353, 1.0)
			else
				questRecord.QuestIcon:SetTexCoord(unpack(QC_ICON_COORDS_NORMAL))
				questRecord.QuestName:SetTextColor(1.0, 1.0, 1.0, 1.0)
			...
			if (qcCompletedQuests[questId]) then
				if not ((questType == 2) or (questType == 4) or (questType == 128) or (questType == 256)) then
	...
	for qcIndex, qcEntry in pairs(qcQuestDatabase) do
		...
		if (C_QuestLog.IsQuestFlaggedCompleted(qcIndex)) then
			if not (qcQuestDatabase[qcIndex][6] == 2) or 
				   ...
				   (qcQuestDatabase[qcIndex][6] == 128) or 				   
				   (qcQuestDatabase[qcIndex][6] == 256) then
				if (qcCompletedQuests[qcIndex] == nil) then
					qcNewFlagged = (qcNewFlagged + 1)ww
				end
...
function qcUpdateTooltip(index)
	...
	if not (questId == nil) then
		...
		if not (qcQuestDatabase[questId][12] == nil) then
			...
			for qcReputationIndex, qcReputationEntry in pairs{qcQuestDatabase[questId][12]} do -- Beta entry *Control what , seperated cvs value  Reputation changes dialog uses*
				...
				for qcInitiatorIndex, qcInitiatorEntry in pairs(qcPinDB[qcMapIndex]) do
					for qcInitiatorQuestIndex, qcInitiatorQuestEntry in pairs(qcPinDB[qcMapIndex][qcInitiatorIndex][7]) do			
						if (qcInitiatorQuestEntry == qcQuestID) then
							...
							--TomTom:AddWaypoint(qcMapIndex, 0, qcInitiatorEntry[5], qcInitiatorEntry[6], qcInitiatorEntry[4])
							--addedWayPoint = true
							break
						end
					end

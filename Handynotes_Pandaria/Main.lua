HandyNotes_Pandaria = LibStub("AceAddon-3.0"):NewAddon("HandyNotes_Pandaria", "AceBucket-3.0", "AceEvent-3.0", "AceTimer-3.0")
local HandyNotes = LibStub("AceAddon-3.0"):GetAddon("HandyNotes", true)

if not HandyNotes then return end

local iconDefaults = {
    default = "Interface\\Addons\\HandyNotes_Pandaria\\Artwork\\Chest.blp",
    unknown = "Interface\\Addons\\HandyNotes_Pandaria\\Artwork\\Chest.blp",
    skull_grey = "Interface\\Addons\\HandyNotes_Pandaria\\Artwork\\Skull-White.blp",
	ritual_stone = "Interface\\Icons\\inv_qiraj_jewelglyphed",
}

local PlayerFaction, _ = UnitFactionGroup("player")
HandyNotes_Pandaria.nodes = { }

local nodes = HandyNotes_Pandaria.nodes
...
function HandyNotes_Pandaria:OnEnter(mapFile, coord)
	...
	if (nodes[mapFile][coord][4] ~= "") and (HandyNotes_Pandaria.db.profile.show_notes == true) then
		tooltip:AddLine(" ")
		tooltip:AddLine("Additional Information")
		tooltip:AddLine(nodes[mapFile][coord][4], 1, 1, 1, 1)
	...
	if (nodes[mapFile][coord][3] ~= nil) and (HandyNotes_Pandaria.db.profile.show_loot == true) and (string.find(nodes[mapFile][coord][6], "quests") == nil) and (nodes[mapFile][coord][6] ~= "rare_ach") then
        if ((nodes[mapFile][coord][7] ~= nil) and (nodes[mapFile][coord][7] ~= "")) then
			...
			tooltip:AddLine(nodes[mapFile][coord][3], 1, 1, 1, 1)
        end
    end
...
local HandyNotes_PandariaDropdownMenu = CreateFrame("Frame", "HandyNotes_PandariaDropdownMenu")
HandyNotes_PandariaDropdownMenu.displayMode = "MENU_Pandaria"
HandyNotes_PandariaDropdownMenu.initialize = generateMenu

function HandyNotes_Pandaria:OnClick(button, down, mapFile, coord)
    if button == "RightButton" and down then
        clickedMapFile = mapFile
        clickedCoord = coord
        ToggleDropDownMenu(1, nil, HandyNotes_PandariaDropdownMenu, self, 0, 0)
    end
end

function HandyNotes_Pandaria:OnLeave(mapFile, coord)
...
local options = {
    ...
    get = function(info) return HandyNotes_Pandaria.db.profile[info.arg] end,
    set = function(info, v) HandyNotes_Pandaria.db.profile[info.arg] = v; HandyNotes_Pandaria:Refresh() end,
    args = {
        desc = {
        ...
        VisibilityOptions = {
            ...
            args = {
                ...
                alwaysshowrares = {
                    type = "toggle",
                    arg = "alwaysshowrares",
                    name = "Show all rares with toys (requires reload)",
				...
				alwaysshowbosses = {
					...
					name = "Show looted World Bosses (Sha of Anger, Nalak, Galleon) (requires reload)",
				...
				low_impact = {
					type = "toggle",
					arg = "low_impact",
					name = "Low Impact (requires reload)",
...
function HandyNotes_Pandaria:OnInitialize()
    local defaults = {
        profile = {
            icon_scale_treasures = 1.0,
            icon_scale_rares = 1.0,
            icon_alpha = 1.00,
    ...
    self.db = LibStub("AceDB-3.0"):New("HandyNotes_PandariaDB", defaults, "Default")
    self:RegisterEvent("PLAYER_ENTERING_WORLD", "WorldEnter")
end

function HandyNotes_Pandaria:WorldEnter()
...
function HandyNotes_Pandaria:QuestCheck()
    do
		if (C_QuestLog.IsQuestFlaggedCompleted(32099) == false) or (self.db.profile.alwaysshowbosses) then
            nodes["KunLaiSummit"][67607460] = { "960009", "Sha of Anger", "Mount.", "Weekly cooldown.", "default", "boss_sha", "87771"}
        end
		
		if (C_QuestLog.IsQuestFlaggedCompleted(32098) == false) or (self.db.profile.alwaysshowbosses) then
			nodes["ValleyoftheFourWinds"][71606440] = { "960010", "Galleon", "Mount.", "Weekly cooldown.", "default", "boss_galleon", "89783"}
		end
		
		if (C_QuestLog.IsQuestFlaggedCompleted(32518) == false) or (self.db.profile.alwaysshowbosses) then
...
function HandyNotes_Pandaria:AchievementCheck()
	do
		local zones = {
		["rare_dw"] = "DreadWastes",
...
function HandyNotes_Pandaria:RegisterWithHandyNotes()
    do
        local function iter(t, prestate)
            ...
            while state do
                ...
                if (value[1] and self.db.profile[value[6]] and not HandyNotes_Pandaria:HasBeenLooted(value)) and (value[6] ~= "rare_h_tj") then
                    ...
                    if ((value[5] == "default") or (value[5] == "unknown")) then
                        if ((value[7] ~= nil) and (value[7] ~= "")) then
                            return state, nil, GetIcon(value[7]), HandyNotes_Pandaria.db.profile.icon_scale_treasures, HandyNotes_Pandaria.db.profile.icon_alpha
                        else
                            return state, nil, iconDefaults[value[5]], HandyNotes_Pandaria.db.profile.icon_scale_treasures, HandyNotes_Pandaria.db.profile.icon_alpha
                        end
                    end

                    return state, nil, iconDefaults[value[5]], HandyNotes_Pandaria.db.profile.icon_scale_rares, HandyNotes_Pandaria.db.profile.icon_alpha
                end
        ...
        function HandyNotes_Pandaria:GetNodes(mapFile, isMinimapUpdate, dungeonLevel)
            return iter, nodes[mapFile], nil
        end
    end

    HandyNotes:RegisterPluginDB("HandyNotes_Pandaria", self, options)
    self:RegisterBucketEvent({ "LOOT_CLOSED" }, 2, "Refresh")
    self:Refresh()
end
 
function HandyNotes_Pandaria:Refresh()
    self:SendMessage("HandyNotes_NotifyUpdate", "HandyNotes_Pandaria")
end

function ResetDB()
    table.wipe(HandyNotes_Pandaria.db.char)
    HandyNotes_Pandaria:Refresh()
end

function HandyNotes_Pandaria:HasBeenLooted(value)
    if (self.db.profile.alwaysshowbosses and (string.find(value[6], "world_bosses") == nil)) then return false end
    if (HandyNotes_Pandaria.db.char[value[1]] and self.db.profile.save) then return true end
...
function DisableRare(button, mapFile, coord)
    if (nodes[mapFile][coord][1] ~= nil) then
        HandyNotes_Pandaria.db.char[nodes[mapFile][coord][1]] = true;
    end
...
function addtoTomTom(button, mapFile, coord)
    if isTomTomloaded == true then
        ...
        if (nodes[mapFile][coord][3] ~= nil) and (HandyNotes_Pandaria.db.profile.show_loot == true) then
            if ((nodes[mapFile][coord][7] ~= nil) and (nodes[mapFile][coord][7] ~= "")) then

...
-- Note: Death Grip, Provoke, and Leer of the Ox have different spell IDs
-- for the spell cast and the applied aura. The Shaman Earth Elemental AoE
-- taunt, Angered Earth (36213), does not apply an aura, so I left it out
-- to avoid immune spam on bosses.
local tauntAuras = {
	[355]=1,	-- Warrior, Taunt
	[2649]=1,	-- Hunter pet, Growl
	[6795]=1,	-- Druid, Growl
	[17735]=1,	-- Warlock Voidwalker, Suffering
	[20736]=1,	-- Hunter, Distracting Shot
	[51399]=1,	-- Death Knight, Death Grip (spell 49576)
	[56222]=1,	-- Death Knight, Dark Command
	[62124]=1,	-- Paladin, Hand of Reckoning
	[73684]=1,	-- Shaman, Unleash Earth
	[97827]=1,	-- Warlock, Provocation (Dark Apotheosis)
	[114198]=1,	-- Warrior, Mocking Banner
	[116189]=1,	-- Monk, Provoke (spell 115546)
...
-- Note: The Shaman Unleash Earth taunt is applied with Unleash Elements, so
-- we cannot check against the spell without triggering on other types.
local tauntSpells = {
	[355]=1,	-- Warrior, Taunt
	[2649]=1,	-- Hunter pet, Growl
	[6795]=1,	-- Druid, Growl
	[17735]=1,	-- Warlock Voidwalker, Suffering
	[20736]=1,	-- Hunter, Distracting Shot
	[49576]=1,	-- Death Knight, Death Grip (aura 51399)
	[56222]=1,	-- Death Knight, Dark Command
	[62124]=1,	-- Paladin, Hand of Reckoning
	[97827]=1,	-- Warlock, Provocation (Dark Apotheosis)
	[114198]=1,	-- Warrior, Mocking Banner
...
--[[
	...
	if subEvent == 'SPELL_CAST_SUCCESS' then
		print((sourceName or '?') .. ' -> ' .. (destName or '?') .. ' spellcast ' .. GetSpellLinkCached(spellID) .. ' with ID ' .. spellID .. ' flags ' .. bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MASK))
	elseif subEvent == 'SPELL_AURA_APPLIED' then
		print((sourceName or '?') .. ' -> ' .. (destName or '?') ..  ' aura ' .. GetSpellLinkCached(spellID) .. ' with ID ' .. spellID .. ' flags ' .. bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MASK))
	end
--]]

	-- Check if event was a taunt aura applied or taunt spell missed
	-- Note: We could use SPELL_CAST_SUCCESS instead, but it has a few issues,
	-- like the Shaman taunt being cast through Unleash Elements. Also,
	-- casting an AoE taunt does not mean it actually taunts anything.
	-- Checking for the aura will be slightly slower though, because there
	-- are more of those events.
	if subEvent == 'SPELL_AURA_APPLIED' then
		-- If applied aura was not a taunt, return
		if not tauntAuras[spellID] then return end
	elseif subEvent == 'SPELL_MISSED' then
		-- If missed spell was not a taunt, return
		if not (tauntSpells[spellID] or tauntAuras[spellID]) then return end
	...
	-- Special handling of Mocking Banner
	-- Note: Since mocking banner potentially taunts every second for 30
	-- seconds, we limit the announcing to the first taunt for each player.
	if spellID == 114198 then
		-- Get time of last Mocking Banner taunt announced for this source
		local lastMB = mockingBanners[sourceName]
		local currentMB = time()

		-- If we already announced a Mocking Banner taunt for this source
		-- within the last 40 seconds, return
		-- Note: While Mocking Banner only lasts 30 seconds, the cooldown
		-- is 3 minutes, so we check for a liberal 40 second window.
		if lastMB and currentMB <= lastMB + 40 then
			return
		end

		-- Add, or update, the last Mocking Banner taunt for this source
		mockingBanners[sourceName] = currentMB
	end

if GetLocale() ~= "ptBR" then return end
local L

--------------------------------
-- Blackfathom Deeps (1) --
--------------------------------
--  Ghamoo-Ra  --
-----------------------------
L = DBM:GetModLocalization("GhamooRa")

L:SetGeneralLocalization{
	name 		= "Ghamoo-Ra"
}
-----------------------------
--  Lady Sarevess  --
-----------------------------
L = DBM:GetModLocalization("LadySerevess")

L:SetGeneralLocalization{
	name 		= "Lady Sarevess"
}
-----------------------------
--  Gelihast  --
-----------------------------
L = DBM:GetModLocalization("Gelihast")

L:SetGeneralLocalization{
	name 		= "Gelihast"
}
-----------------------------
--  Old Serra'kis  --
-----------------------------
L = DBM:GetModLocalization("OldSerrakis")

L:SetGeneralLocalization{
	name 		= "Velho Serra'kis"
}
-----------------------------
--  Twilight Lord Kelris  --
-----------------------------
L = DBM:GetModLocalization("TwilightLordKelris")

L:SetGeneralLocalization{
	name 		= "Senhor do Crepúsculo Kelris"
}
-----------------------------
--  Aku'mai  --
-----------------------------
L = DBM:GetModLocalization("Akumai")

L:SetGeneralLocalization{
	name 		= "Aku'mai"
}

-------------------------------------------------
-- Blackrock Depths - 228J/230M (2) --
-------------------------------------------------
-- High Interrogator Gerstahn --
---------------------------------
L = DBM:GetModLocalization(369)

L:SetGeneralLocalization{
	name 		= "Suprema Interrogadora Gerstahn"
}
-----------------
-- Lord Roccor --
-----------------
L = DBM:GetModLocalization(370)

L:SetGeneralLocalization{
	name 		= "Lorde Pétror"
}
------------------------------
-- Hound Master Grebmar --
------------------------------
L = DBM:GetModLocalization(371)

L:SetGeneralLocalization{
	name 		= "Mestre de Matilha Grebmar"
}
---------------------------------------------------------
-- Ring of Law --
---------------------------------------------------------
L = DBM:GetModLocalization(372)

L:SetGeneralLocalization{
	name 		= "Círculo da Lei"
}
---------------------------------
-- Pyromancer Loregrain --
---------------------------------
L = DBM:GetModLocalization(373)

L:SetGeneralLocalization{
	name 		= "Piromante Sábia Semente"
}
--------------------
-- Lord Incendius --
--------------------
L = DBM:GetModLocalization(374)

L:SetGeneralLocalization{
	name 		= "Lorde Incendius"
}
--------------------------
-- Warder Stilgiss --
--------------------------
L = DBM:GetModLocalization(375)

L:SetGeneralLocalization{
	name 		= "Carcereiro Stilgiss"
}
----------------------
-- Fineous Darkvire --
----------------------
L = DBM:GetModLocalization(376)

L:SetGeneralLocalization{
	name 		= "Fineous Forçanegra"
}
--------------
-- Bael'Gar --
--------------
L = DBM:GetModLocalization(377)

L:SetGeneralLocalization{
	name 		= "Bael'gar"
}
--------------------------
-- General Angerforge --
--------------------------
L = DBM:GetModLocalization(378)

L:SetGeneralLocalization{
	name 		= "General Forjaversa"
}
---------------------------
-- Golem Lord Argelmach --
---------------------------
L = DBM:GetModLocalization(379)

L:SetGeneralLocalization{
	name 		= "Lorde Golem Argelmach"
}
----------------------
-- Hurley Blackbreath --
----------------------
L = DBM:GetModLocalization(380)

L:SetGeneralLocalization{
	name 		= "Hurley Bafonegro"
}
-------------
-- Phalanx --
-------------
L = DBM:GetModLocalization(381)

L:SetGeneralLocalization{
	name 		= "Falange"
}
-----------------------
-- Plugger Spazzring --
-----------------------
L = DBM:GetModLocalization(383)

L:SetGeneralLocalization{
	name 		= "Birra Guardachave"
}
-------------------------
-- Ambassador Flamelash --
-------------------------
L = DBM:GetModLocalization(384)

L:SetGeneralLocalization{
	name 		= "Embaixador Fogaçoite"
}
---------------
-- The Seven --
---------------
L = DBM:GetModLocalization(385)

L:SetGeneralLocalization{
	name 		= "Os Sete"
}
------------
-- Magmus --
------------
L = DBM:GetModLocalization(386)

L:SetGeneralLocalization{
	name 		= "Magmus"
}
---------------------------------
-- Emperor Dagran Thaurissan --
---------------------------------
L = DBM:GetModLocalization(387)

L:SetGeneralLocalization{
	name 		= "Imperador Dagran Thaurissan"
}

-------------------------
--  Lower Blackrock Spire-229J/229M (6L/3C)  --
--  Highlord Omokk  --
-----------------------------
L = DBM:GetModLocalization(388)

L:SetGeneralLocalization{
	name 		= "Grão-lorde Omokk"
}
-----------------------------
--  Shadow Hunter Vosh'gajin  --
-----------------------------
L = DBM:GetModLocalization(389)

L:SetGeneralLocalization{
	name 		= "Caçadora Sombria Vosh'gajin"
}
-----------------------------
--  War Master Voone  --
-----------------------------
L = DBM:GetModLocalization(390)

L:SetGeneralLocalization{
	name 		= "Senhor da Guerra Voone"
}
-----------------------------
--  Mother Smolderweb  --
-----------------------------
L = DBM:GetModLocalization(391)

L:SetGeneralLocalization{
	name 		= "Mãe Queimateia"
}
-----------------------------
--  Urok Doomhowl  --
-----------------------------
L = DBM:GetModLocalization(392)

L:SetGeneralLocalization{
	name 		= "Urok Uivo-da-ruína"
}
-----------------------------
--  Quartermaster Zigris  --
-----------------------------
L = DBM:GetModLocalization(393)

L:SetGeneralLocalization{
	name 		= "Intendente Zigris"
}
-----------------------------
--  Halycon  --
-----------------------------
L = DBM:GetModLocalization(394)

L:SetGeneralLocalization{
	name 		= "Halycon"
}
-----------------------------
--  Gizrul  --
-----------------------------
L = DBM:GetModLocalization(395)

L:SetGeneralLocalization{
	name 		= "Gizrul, o Subjugador"
}
-----------------------------
--  Overlord Wyrmthalak  --
-----------------------------
L = DBM:GetModLocalization(396)

L:SetGeneralLocalization{
	name 		= "Lorde Supremo Wyrmthalak"
}

-------------------------
--  Upper Blackrock Spire-229J/229M (4C)  --
-----------------------------
--  Pyroguard Emberseer  --
-----------------------------
L = DBM:GetModLocalization("PyroguardEmberseer")

L:SetGeneralLocalization{
	name 		= "Piroguarda Mirabrasa"
}
L:SetMiscLocalization{
	Pull		= "%s começa a recuperar a força!"
}
-----------------------------
--  Solakar Flamewreath  --
-----------------------------
L = DBM:GetModLocalization("SolakarFlamewreath")

L:SetGeneralLocalization{
	name 		= "Solakar Chamarco"
}
-----------------------------
--  Goraluk Anvilcrack  --
-----------------------------
L = DBM:GetModLocalization("GoralukAnvilcrack")

L:SetGeneralLocalization{
	name 		= "Goraluk Rachadastra"
}
-----------------------------
--  Jed Runewatcher  --
-----------------------------
L = DBM:GetModLocalization("JedRunewatcher")

L:SetGeneralLocalization{
	name 		= "Jed Mirarruna"
}
-----------------------------
--  Warchief Rend Blackhand  --
-----------------------------
L = DBM:GetModLocalization("WarchiefRendBlackhand")

L:SetGeneralLocalization{
	name 		= "Chefe Guerreiro Laceral Mão Negra"
}
-----------------------------
--  The Beast  --
-----------------------------
L = DBM:GetModLocalization("TheBeast")

L:SetGeneralLocalization{
	name 		= "A Fera"
}
----------------------
--  Lord Valthalak  --
----------------------
L = DBM:GetModLocalization("LordValthalak")

L:SetGeneralLocalization{
	name 		= "Lorde Valthalak"
}
-----------------------------
--  General Drakkisath  --
-----------------------------
L = DBM:GetModLocalization("GeneralDrakkisath")

L:SetGeneralLocalization{
	name 		= "General Drakkisath"
}

-------------------------
--  Deadmines (3L/5C)  --
-----------------------------
--  Rhahk'Zor  --
-----------------------------
L = DBM:GetModLocalization("RhahkZor")

L:SetGeneralLocalization{
	name 		= "Rhahk'Zor"
}
-----------------------------
--  Miner Johnson  --
-----------------------------
L = DBM:GetModLocalization("MinerJohnson")

L:SetGeneralLocalization{
	name 		= "Minerador João"
}
-----------------------------
--  Sneed's Shredder  --
-----------------------------
L = DBM:GetModLocalization("SneedsShredder")

L:SetGeneralLocalization{
	name 		= "Retalhador do Sneed"
}
-----------------------------
--  Gilnid  --
-----------------------------
L = DBM:GetModLocalization("Gilnid")

L:SetGeneralLocalization{
	name 		= "Gilnid"
}
-----------------------------
--  Mr. Smite  --
-----------------------------
L = DBM:GetModLocalization("MrSmite")

L:SetGeneralLocalization{
	name 		= "Sr. Castiga"
}
-----------------------------
--  Captain Greenskin  --
-----------------------------
L = DBM:GetModLocalization("CaptainGreenskin")

L:SetGeneralLocalization{
	name 		= "Capitão Peleverde"
}
-----------------------------
--  Edwin Van Cleef  --
-----------------------------
L = DBM:GetModLocalization("EdwinVanCleef")

L:SetGeneralLocalization{
	name 		= "Edwin VanCleef"
}
-----------------------------
--  Cookie  --
-----------------------------
L = DBM:GetModLocalization("Cookie")

L:SetGeneralLocalization{
	name 		= "Cuca"
}
--------------------------------
-- Dire Maul - 230J/429M (3) --
--------------------------------
-- Zevrim Thornhoof --
--------------------------
L = DBM:GetModLocalization(402)

L:SetGeneralLocalization{
	name 		= "Zevrim Cascardo"
}
------------------
-- Hydrospawn --
------------------
L = DBM:GetModLocalization(403)

L:SetGeneralLocalization{
	name 		= "Rebentágua"
}
-----------------
-- Lethtendris --
-----------------
L = DBM:GetModLocalization(404)

L:SetGeneralLocalization{
	name 		= "Letendris"
}
--------------------------
-- Alzzin the Wildshaper --
--------------------------
L = DBM:GetModLocalization(405)

L:SetGeneralLocalization{
	name 		= "Azzin, o Selvamorfo"
}
--------------------
-- Tendris Warpwood --
--------------------
L = DBM:GetModLocalization(406)

L:SetGeneralLocalization{
	name 		= "Gavíneo Lenhatorta"
}
--------------------------
-- Illyanna Ravenoak --
--------------------------
L = DBM:GetModLocalization(407)

L:SetGeneralLocalization{
	name 		= "Illyanna Corvalho"
}
------------------------
-- Magister Kalendris --
------------------------
L = DBM:GetModLocalization(408)

L:SetGeneralLocalization{
	name 		= "Magíster Kalendris"
}
----------------
-- Immol'thar --
----------------
L = DBM:GetModLocalization(409)

L:SetGeneralLocalization{
	name 		= "Immol'thar"
}
--------------------------
-- Prince Tortheldrin --
--------------------------
L = DBM:GetModLocalization(410)

L:SetGeneralLocalization{
	name 		= "Príncipe Tortheldrin"
}
---------------------
-- Guard Mol'dar --
---------------------
L = DBM:GetModLocalization(411)

L:SetGeneralLocalization{
	name 		= "Guarda Mol'dar"
}
----------------------
-- Stomper Kreeg --
----------------------
L = DBM:GetModLocalization(412)

L:SetGeneralLocalization{
	name 		= "Pisoteador Kreeg"
}
--------------------
-- Guard Fengus --
--------------------
L = DBM:GetModLocalization(413)

L:SetGeneralLocalization{
	name 		= "Guarda Fengus"
}
----------------------
-- Guard Slip'kik --
----------------------
L = DBM:GetModLocalization(414)

L:SetGeneralLocalization{
	name 		= "Guarda Kishutt"
}
-----------------------
-- Captain Kromcrush --
-----------------------
L = DBM:GetModLocalization(415)

L:SetGeneralLocalization{
	name 		= "Capitão Kebrapaw"
}
----------------------------
-- Cho'Rush the Observer --
----------------------------
L = DBM:GetModLocalization(416)

L:SetGeneralLocalization{
	name 		= "Ez'Magg, o Observador"
}
----------------
-- King Gordok --
----------------
L = DBM:GetModLocalization(417)

L:SetGeneralLocalization{
	name 		= "Rei Gordok"
}

-------------------------
--  Gnomeregan-231J/90M (5L/7C)  --
-----------------------------
--  Grubbis  --
-----------------------------
L = DBM:GetModLocalization(419)

L:SetGeneralLocalization{
	name 		= "Grúdio"
}
-----------------------------
--  Viscous Fallout  --
-----------------------------
L = DBM:GetModLocalization(420)

L:SetGeneralLocalization{
	name 		= "Precipitação Radioativa Viscosa"
}
-----------------------------
--  Electrocutioner 6000  --
-----------------------------
L = DBM:GetModLocalization(421)

L:SetGeneralLocalization{
	name 		= "Eletrocutor 6000"
}
-----------------------------
--  Crowd Pummeler 9-60  --
-----------------------------
L = DBM:GetModLocalization(418)

L:SetGeneralLocalization{
	name 		= "Espanca-gente 9-60"
}
-----------------------------
--  Mekgineer Thermaplugg  --
-----------------------------
L = DBM:GetModLocalization(422)

L:SetGeneralLocalization{
	name 		= "Mecangenheiro Termaplugue"
}

-------------------------
--  Maraudon-232J/349M (6L/8C)  --
-----------------------------
--  Noxxion  --
-----------------------------
L = DBM:GetModLocalization(423)

L:SetGeneralLocalization{
	name 		= "Tóxxico"
}

-----------------------------
--  Razorlash  --
-----------------------------
L = DBM:GetModLocalization(424)

L:SetGeneralLocalization{
	name 		= "Azorrague"
}

-----------------------------
--  Tinkerer Gizlock  --
-----------------------------
L = DBM:GetModLocalization(425)

L:SetGeneralLocalization{
	name 		= "Engenhoqueiro Bugitranca"
}

-----------------------------
--  Lord Vyletongue  --
-----------------------------
L = DBM:GetModLocalization(427)

L:SetGeneralLocalization{
	name 		= "Lorde Torpelíngua"
}

-----------------------------
--  Celebras the Cursed  --
-----------------------------
L = DBM:GetModLocalization(428)

L:SetGeneralLocalization{
	name 		= "Celebras, o Amaldiçoado"
}

-----------------------------
--  Landslide  --
-----------------------------
L = DBM:GetModLocalization(429)

L:SetGeneralLocalization{
	name 		= "Soterrador"
}

-----------------------------
--  Rotgrip  --
-----------------------------
L = DBM:GetModLocalization(430)

L:SetGeneralLocalization{
	name 		= "Putrigarra"
}

-----------------------------
--  Princess Theradras  --
-----------------------------
L = DBM:GetModLocalization(431)

L:SetGeneralLocalization{
	name 		= "Princesa Theradras"
}

-------------------------
--  Ragefire Chasm (7L/9C)  --
-----------------------------
--  Oggleflint  --
-----------------------------
L = DBM:GetModLocalization("Oggleflint")

L:SetGeneralLocalization{
	name 		= "Pederneiro"
}
-----------------------------
--  Taragaman the Hungerer  --
-----------------------------
L = DBM:GetModLocalization("Taragaman")

L:SetGeneralLocalization{
	name 		= "Taragaman, o Famélico"
}
-----------------------------
--  Jergosh the Invoker  --
-----------------------------
L = DBM:GetModLocalization("Jergosh")

L:SetGeneralLocalization{
	name 		= "Jergosh, o Invocador"
}
-----------------------------
--  Bazzalan  --
-----------------------------
L = DBM:GetModLocalization("Bazzalan")

L:SetGeneralLocalization{
	name 		= "Bazzalan"
}

-------------------------
--  Razorfen Downs -233J/129M (8L/10C)  --
-----------------------------
--  Tuten'kash  --
-----------------------------
L = DBM:GetModLocalization("Tutenkash")

L:SetGeneralLocalization{
	name 		= "Tutan'kash"
}

-----------------------------
--  Mordresh Fire Eye  --
-----------------------------
L = DBM:GetModLocalization("MordreshFireEye")

L:SetGeneralLocalization{
	name 		= "Mordresh Olho-de-Fogo"
}

-----------------------------
--  Glutton  --
-----------------------------
L = DBM:GetModLocalization("Glutton")

L:SetGeneralLocalization{
	name 		= "Glutão"
}

-----------------------------
--  Ragglesnout  --
-----------------------------
L = DBM:GetModLocalization("Ragglesnout")

L:SetGeneralLocalization{
	name 		= "Trombarrão"
}

-----------------------------
--  Amnenar the Coldbringer  --
-----------------------------
L = DBM:GetModLocalization("AmnennartheColdbringer")

L:SetGeneralLocalization{
	name 		= "Amnennar, o Frigífero"
}

-----------------------------
--  Plaguemaw the Rotting  --
-----------------------------
L = DBM:GetModLocalization("PlaguemawtheRotting")

L:SetGeneralLocalization{
	name 		= "Pragorja, o Podre"
}

-------------------------
--  Razorfen Kraul -234J/47M (9L/11C)  --
-----------------------------
--  Roogug  --
-----------------------------
L = DBM:GetModLocalization("Roogug")

L:SetGeneralLocalization{
	name 		= "Rugug"
}

-----------------------------
--  Aggem Thorncurse  --
-----------------------------
L = DBM:GetModLocalization("AggemThorncurse")

L:SetGeneralLocalization{
	name 		= "Aggem Pragacardo"
}

-----------------------------
--  Death Speaker Jargba  --
-----------------------------
L = DBM:GetModLocalization("DeathSpeakerJargba")

L:SetGeneralLocalization{
	name 		= "Morta-voz Jargba"
}

-----------------------------
--  Overload Ramtusk  --
-----------------------------
L = DBM:GetModLocalization("OverlordRamtusk")

L:SetGeneralLocalization{
	name 		= "Lorde Supremo Marretusco"
}

-----------------------------
--  Agathelos the Raging  --
-----------------------------
L = DBM:GetModLocalization("AgathelostheRaging")

L:SetGeneralLocalization{
	name 		= "Agathelos, o Furioso"
}

-----------------------------
--  Blind Hunter  --
-----------------------------
L = DBM:GetModLocalization("BlindHunter")

L:SetGeneralLocalization{
	name 		= "Caçador Cego"
}

-----------------------------
--  Earthcaller Halmgar  --
-----------------------------
L = DBM:GetModLocalization("EarthcallerHalmgar")

L:SetGeneralLocalization{
	name 		= "Arauto da Terra Halmgar"
}

-----------------------------
--  Charlga Razorflank  --
-----------------------------
L = DBM:GetModLocalization("CharlgaRazorflank")

L:SetGeneralLocalization{
	name 		= "Charlga Talhaflanco"
}

-------------------------
--  Scarlet Monastery 189M (12C)  --
-----------------------------
--  Interrogator Vishas  --
-----------------------------
L = DBM:GetModLocalization("InterrogatorVishas")

L:SetGeneralLocalization{
	name 		= "Interrogador Vishas"
}

-----------------------------
--  Bloodmage Thalnos  --
-----------------------------
L = DBM:GetModLocalization("BloodmageThalnos")

L:SetGeneralLocalization{
	name 		= "Mago Sangrento Thalnos"
}

-----------------------------
--  Houndmaster Loksey  --
-----------------------------
L = DBM:GetModLocalization("HoundmasterLoksey")

L:SetGeneralLocalization{
	name 		= "Mestre de Matilha Lobato"
}

-----------------------------
--  Arcanist Doan  --
-----------------------------
L = DBM:GetModLocalization("ArcanistDoan")

L:SetGeneralLocalization{
	name 		= "Arcanista Doan"
}

-----------------------------
--  Herod  --
-----------------------------
L = DBM:GetModLocalization("Herod")

L:SetGeneralLocalization{
	name 		= "Herodes"
}

-----------------------------
--  Mograine & Whitemane  --
-----------------------------
L = DBM:GetModLocalization("Mograine_and_Whitemane")

L:SetGeneralLocalization{
	name 		= "Mograine e Cristalba"
}

-----------------------------
--  High Inquisitor Fairbanks  --
-----------------------------
L = DBM:GetModLocalization("Fairbanks")

L:SetGeneralLocalization{
	name 		= "Alto-inquisidor Fairbanks"
}

-------------------------
--  Scholomance 289M (13C)  --
-----------------------------
--  Kirtonos the Herald  --
-----------------------------
L = DBM:GetModLocalization("KirtonostheHerald")

L:SetGeneralLocalization{
	name 		= "Kirtonos, o Arauto"
}

-----------------------------
--  Jandice Barov  --
-----------------------------
L = DBM:GetModLocalization("JandiceBarov")

L:SetGeneralLocalization{
	name 		= "Jandice Barov"
}

-----------------------------
--  Rattlegore  --
-----------------------------
L = DBM:GetModLocalization("Rattlegore")

L:SetGeneralLocalization{
	name 		= "Ossorrange"
}

-----------------------------
--  Marduk Blackpool  --
-----------------------------
L = DBM:GetModLocalization("MardukBlackpool")

L:SetGeneralLocalization{
	name 		= "Marduque Lagonero"
}

-----------------------------
--  Vectus  --
-----------------------------
L = DBM:GetModLocalization("Vectus")

L:SetGeneralLocalization{
	name 		= "Vectus"
}

-----------------------------
--  Ras Frostwhisper  --
-----------------------------
L = DBM:GetModLocalization("RasFrostwhisper")

L:SetGeneralLocalization{
	name 		= "Ras Friomúrmuro"
}

-----------------------------
--  Instructor Malicia  --
-----------------------------
L = DBM:GetModLocalization("InstructorMalicia")

L:SetGeneralLocalization{
	name 		= "Instrutora Malícia"
}

-----------------------------
--  Doctor Theolen Krastinov  --
-----------------------------
L = DBM:GetModLocalization("DoctorTheolenKrastinov")

L:SetGeneralLocalization{
	name 		= "Doutor Theolen Krastinov"
}

-----------------------------
--  Lorekeeper Polkelt  --
-----------------------------
L = DBM:GetModLocalization("LorekeeperPolkelt")

L:SetGeneralLocalization{
	name 		= "Erudito Pulquério"
}

-----------------------------
--  The Ravenian  --
-----------------------------
L = DBM:GetModLocalization("TheRavenian")

L:SetGeneralLocalization{
	name 		= "O Corvino"
}

-----------------------------
--  Lord Alexei Barov  --
-----------------------------
L = DBM:GetModLocalization("LordAlexeiBarov")

L:SetGeneralLocalization{
	name 		= "Lorde Alexei Barov"
}

-----------------------------
--  Lady Illucia Barov  --
-----------------------------
L = DBM:GetModLocalization("LadyIlluciaBarov")

L:SetGeneralLocalization{
	name 		= "Lady Ilúcia Barov"
}

-----------------------------
--  Darkmaster Gandling  --
-----------------------------
L = DBM:GetModLocalization("DarkmasterGandling")

L:SetGeneralLocalization{
	name 		= "Umbromestre Gandling"
}
-------------------------
--  Shadowfang Keep 33M (14C)  --
-----------------------------
--  Rethilgore  --
-----------------------------
L = DBM:GetModLocalization("Rethilgore")

L:SetGeneralLocalization{
	name 		= "Rethilgore"
}

-----------------------------
--  Razorclaw the Butcher  --
-----------------------------
L = DBM:GetModLocalization("RazorclawtheButcher")

L:SetGeneralLocalization{
	name 		= "Garraguda, o Açougueiro"
}

-----------------------------
--  Baron Silverlaine  --
-----------------------------
L = DBM:GetModLocalization("BaronSilverlaine")

L:SetGeneralLocalization{
	name 		= "Barão Silverlaine"
}

-----------------------------
--  Commander Springvale  --
-----------------------------
L = DBM:GetModLocalization("CommanderSpringvale")

L:SetGeneralLocalization{
	name 		= "Comandante Floraval"
}

-----------------------------
--  Odo the Blindwatcher  --
-----------------------------
L = DBM:GetModLocalization("OdotheBlindwatcher")

L:SetGeneralLocalization{
	name 		= "Odo, o Vigia Cego"
}

-----------------------------
--  Deathsworn Captain  --
-----------------------------
L = DBM:GetModLocalization("DeathswornCaptain")

L:SetGeneralLocalization{
	name 		= "Capitão Devoto da Morte"
}

-----------------------------
--  Fenrus the Devourer  --
-----------------------------
L = DBM:GetModLocalization("FenrustheDevourer")

L:SetGeneralLocalization{
	name 		= "Fenrus, o Devorador"
}

-----------------------------
--  Wolf Master Nandos  --
-----------------------------
L = DBM:GetModLocalization("WolfMasterNandos")

L:SetGeneralLocalization{
	name 		= "Nandos, o Mestre de Lobos"
}

-----------------------------
--  Archmage Arugal  --
-----------------------------
L = DBM:GetModLocalization("ArchmageArugal")

L:SetGeneralLocalization{
	name 		= "Arquimago Arugal"
}

-------------------------
--  Stormwind Stockade 34M (10L/15C)  --
-----------------------------
--  Targorr the Dread  --
-----------------------------
L = DBM:GetModLocalization("Targorr")

L:SetGeneralLocalization{
	name 		= "Targorr, o Horror"
}
-----------------------------
--  Kam Deepfury  --
-----------------------------
L = DBM:GetModLocalization("KamDeepfury")

L:SetGeneralLocalization{
	name 		= "Kam Fundafúria"
}
-----------------------------
--  Hamhock  --
-----------------------------
L = DBM:GetModLocalization("Hamhock")

L:SetGeneralLocalization{
	name 		= "Ramoque"
}
-----------------------------
--  Bazil Thredd  --
-----------------------------
L = DBM:GetModLocalization("BazilThredd")

L:SetGeneralLocalization{
	name 		= "Basílio Taborda"
}
-----------------------------
--  Dextren Ward  --
-----------------------------
L = DBM:GetModLocalization("DextrenWard")

L:SetGeneralLocalization{
	name 		= "Flávio Lúcio"
}
-----------------------------
--  Bruegal Ironknuckle  --
-----------------------------
L = DBM:GetModLocalization("BruegalIronknuckle")

L:SetGeneralLocalization{
	name 		= "Bruegal Ferroque"
}

-------------------------
--  Stratholme 236J/329M (10L/16C)  --
-----------------------------
--  Hearthsinger Forresten  --
-----------------------------
L = DBM:GetModLocalization(443)

L:SetGeneralLocalization{
	name 		= "Cantalar Forresten"
}

-----------------------------
--  Timmy the Cruel  --
-----------------------------
L = DBM:GetModLocalization(445)

L:SetGeneralLocalization{
	name 		= "Tico, o Cruel"
}

-----------------------------
--  Malor the Zealous  --
-----------------------------
L = DBM:GetModLocalization(749)

L:SetGeneralLocalization{
	name 		= "Malor, o Zeloso"
}

-----------------------------
--  Cannon Master Willey  --
-----------------------------
L = DBM:GetModLocalization(446)

L:SetGeneralLocalization{
	name 		= "Mestre de Canhões Gualter"
}

-----------------------------
--  Archivist Galford  --
-----------------------------
L = DBM:GetModLocalization(448)

L:SetGeneralLocalization{
	name 		= "Arquivista Galford"
}

-----------------------------
--  Balnazzar  --
-----------------------------
L = DBM:GetModLocalization(449)

L:SetGeneralLocalization{
	name 		= "Balnazzar"
}

-----------------------------
--  The Unforgiven  --
-----------------------------
L = DBM:GetModLocalization(450)

L:SetGeneralLocalization{
	name 		= "O Imperdoável"
}

-----------------------------
--  Baroness Anastari  --
-----------------------------
L = DBM:GetModLocalization(451)

L:SetGeneralLocalization{
	name 		= "Baronesa Anastari"
}

-----------------------------
--  Nerub'enkan  --
-----------------------------
L = DBM:GetModLocalization(452)

L:SetGeneralLocalization{
	name 		= "Nerub'enkan"
}

-----------------------------
--  Maleki the Pallid  --
-----------------------------
L = DBM:GetModLocalization(453)

L:SetGeneralLocalization{
	name 		= "Malaki, o Pálido"
}

-----------------------------
--  Magistrate Barthilas  --
-----------------------------
L = DBM:GetModLocalization(454)

L:SetGeneralLocalization{
	name 		= "Magistrado Barthilas"
}

-----------------------------
--  Ramstein the Gorger  --
-----------------------------
L = DBM:GetModLocalization(455)

L:SetGeneralLocalization{
	name 		= "Ramstein, o Devorador"
}
L:SetTimerLocalization{
	TimerGuards	= "Guardas aparecem"
}
L:SetOptionLocalization{
	TimerGuards	= "Depois de derrotar o chefe, exibe o cronômetro para a chegada das Sentinelas da Guarda Negra"
}
-----------------------------
--  Baron Rivendare  --
-----------------------------
L = DBM:GetModLocalization(456)

L:SetGeneralLocalization{
	name 		= "Barão Rivendare"
}

-------------------------
--  Sunken Temple 237J/109M (11L/17C)  --
-----------------------------
--  Avatar of Hakkar  --
-----------------------------
L = DBM:GetModLocalization(457)

L:SetGeneralLocalization{
	name 		= "Avatar de Hakkar"
}
-----------------------------
--  Jammal'an the Prophet  --
-----------------------------
L = DBM:GetModLocalization(458)

L:SetGeneralLocalization{
	name 		= "Jammal'an, o Profeta"
}
----------------------------------------------
--	Wardens of the Dream					--
--	(Dreamscythe, Weaver, Morphaz, Hazzas)	--
----------------------------------------------
L = DBM:GetModLocalization("Dreamscythe")

L:SetGeneralLocalization{
	name 		= "Darquimeros"
}

L = DBM:GetModLocalization("Weaver")

L:SetGeneralLocalization{
	name 		= "Tecelão"
}

L = DBM:GetModLocalization("Morphaz")

L:SetGeneralLocalization{
	name 		= "Morphaz"
}

L = DBM:GetModLocalization("Hazzas")

L:SetGeneralLocalization{
	name 		= "Hazzas"
}

-----------------------------
--  Shade of Eranikus  --
-----------------------------
L = DBM:GetModLocalization(463)

L:SetGeneralLocalization{
	name 		= "Vulto de Erânicos"
}

-------------------------
--  Uldaman 229J/70M (12L/18C)  --
-----------------------------
--  Revelosh  --
-----------------------------
L = DBM:GetModLocalization(467)

L:SetGeneralLocalization{
	name 		= "Revelosh"
}
-----------------------------
--  The Lost Dwarves  --
-----------------------------
L = DBM:GetModLocalization(468)

L:SetGeneralLocalization{
	name 		= "Os anões perdidos"
}
-----------------------------
--  Ironaya  --
-----------------------------
L = DBM:GetModLocalization(469)

L:SetGeneralLocalization{
	name 		= "Ferronaya"
}
-----------------------------
--  Obsidian Sentinel  --
-----------------------------
L = DBM:GetModLocalization(748)

L:SetGeneralLocalization{
	name 		= "Sentinela de Obsidiana"
}
-----------------------------
--  Ancient Stone Keeper  --
-----------------------------
L = DBM:GetModLocalization(470)

L:SetGeneralLocalization{
	name 		= "Antigo Protetor de Pedra"
}
-----------------------------
--  Galgann Firehammer  --
-----------------------------
L = DBM:GetModLocalization(471)

L:SetGeneralLocalization{
	name 		= "Galnann Martelígneo"
}
-----------------------------
--  Grimlok  --
-----------------------------
L = DBM:GetModLocalization(472)

L:SetGeneralLocalization{
	name 		= "Grimlok"
}
-----------------------------
--  Archaedas  --
-----------------------------
L = DBM:GetModLocalization(473)

L:SetGeneralLocalization{
	name 		= "Arkhaedas"
}

-------------------------
--  Wailing Caverns 240J/43M (13L/19C)  --
-----------------------------
--  Lady Anacondra  --
-----------------------------
L = DBM:GetModLocalization(474)

L:SetGeneralLocalization{
	name 		= "Lady Sucurina"
}

-----------------------------
--  Lord Pythas  --
-----------------------------
L = DBM:GetModLocalization(476)

L:SetGeneralLocalization{
	name 		= "Lorde Pítias"
}

-----------------------------
--  Lord Cobrahn  --
-----------------------------
L = DBM:GetModLocalization(475)

L:SetGeneralLocalization{
	name 		= "Lorde Cobrahn"
}

-----------------------------
--  Kresh  --
-----------------------------
L = DBM:GetModLocalization(477)

L:SetGeneralLocalization{
	name 		= "Kresh"
}

-----------------------------
--  Skum  --
-----------------------------
L = DBM:GetModLocalization(478)

L:SetGeneralLocalization{
	name 		= "Skória"
}

-----------------------------
--  Lord Serpentis  --
-----------------------------
L = DBM:GetModLocalization(479)

L:SetGeneralLocalization{
	name 		= "Lorde Serpentis"
}

-----------------------------
--  Veran the Everliving  --
-----------------------------
L = DBM:GetModLocalization(480)

L:SetGeneralLocalization{
	name 		= "Verdan, o Sempre-vivo"
}

-----------------------------
--  Mutanus the Devourer  --
-----------------------------
L = DBM:GetModLocalization(481)

L:SetGeneralLocalization{
	name 		= "Mutanus, o Devorador"
}

-----------------------------
--  Deviate Faerie Dragon  --
-----------------------------
L = DBM:GetModLocalization("DeviateFaerie")

L:SetGeneralLocalization{
	name 		= "Dragoleta Anormal"
}

-------------------------
--  Zul'Farrak 241J/209M (14L/20C)  --
-----------------------------
--  Ghaz'rilla  --
-----------------------------
L = DBM:GetModLocalization(483)

L:SetGeneralLocalization{
	name 		= "Gahz'rilla"
}

-----------------------------
--  Antu'sul  --
-----------------------------
L = DBM:GetModLocalization(484)

L:SetGeneralLocalization{
	name 		= "Antu'sul"
}

-----------------------------
--  Theka the Martyr  --
-----------------------------
L = DBM:GetModLocalization(485)

L:SetGeneralLocalization{
	name 		= "Theka, o Mártir"
}

-----------------------------
--  Witch Doctor Zum'rah  --
-----------------------------
L = DBM:GetModLocalization(486)

L:SetGeneralLocalization{
	name 		= "Mandingueiro Zum'rah"
}

-----------------------------
--  Hydromancer Velrath (not in Dungeon Journal)  --
-----------------------------
L = DBM:GetModLocalization("HydromancerVelrath")

L:SetGeneralLocalization{
	name 		= "Hidromante Velratha"
}

-----------------------------
--  Nekrum & Sezz'ziz  --
-----------------------------
L = DBM:GetModLocalization(487)

L:SetGeneralLocalization{
	name 		= "Nekrum e Diz'ieh"
}

-----------------------------
--  Chief Ukorz Sandscalp  --
-----------------------------
L = DBM:GetModLocalization(489)

L:SetGeneralLocalization{
	name 		= "Chefe Ukorz Escalpareia"
}
-----------------------------
--Demon Fall Canyon
-----------------------------
--  Grimroot  --
-----------------------------
L = DBM:GetModLocalization("Grimroot")

L:SetGeneralLocalization{
	name 		= "Broterror"
}

-----------------------------
--  Diathorus the Seeker  --
-----------------------------
L = DBM:GetModLocalization("Diathorus")

L:SetGeneralLocalization{
	name 		= "Diathorus, o Inquisidor"
}
-----------------------------
--  Destructor's Wraith  --
-----------------------------
L = DBM:GetModLocalization("DestructorsWraith")

L:SetGeneralLocalization{
	name 		= "Aparição do Destruidor"
}
-----------------------------
--  Zilbagob  --
-----------------------------
L = DBM:GetModLocalization("Zilbagob")

L:SetGeneralLocalization{
	name 		= "Zilbagob"
}
-----------------------------
--  Azgaloth  --
-----------------------------
L = DBM:GetModLocalization("Azgaloth")

L:SetGeneralLocalization{
	name 		= "Azgaloth"
}
-----------------------------
--  Pyranis  --
-----------------------------
L = DBM:GetModLocalization("Pyranis")

L:SetGeneralLocalization{
	name 		= "Pyranis"
}
-----------------------------
--  Hellscream's Phantom  --
-----------------------------
L = DBM:GetModLocalization("HellscreamsPhantom")

L:SetGeneralLocalization{
	name 		= "Fantasma de Grito Infernal"
}

-------------------------
-- SoD Karazhan Crypts --
-------------------------

-- Encounter info from PTR: https://wago.tools/db2/DungeonEncounter?build=1.15.6.58185&sort[ID]=desc&filter[MapID]=2875&page=1

-- Apprentice, seems to be a randomized boss
L = DBM:GetModLocalization("Apprentice")

L:SetGeneralLocalization{
	name = "Aprendiz"
}

-- Harbinger of Sin
L = DBM:GetModLocalization("HarbingerOfSin")

L:SetGeneralLocalization{
	name = "Emissário do Pecado"
}

-- Kharon
L = DBM:GetModLocalization("Kharon")

L:SetGeneralLocalization{
	name = "Kharon"
}

L:SetWarningLocalization{
	SpecWarnDropTorch = "%d acúmulos - largue a tocha!"
}

-- Opera of Malediction
L = DBM:GetModLocalization("OperaOfMalediction")

L:SetGeneralLocalization{
	name = "Ópera da Maldição"
}

L:SetMiscLocalization {
	PullTrizivast = "For your evenings entertainment, comes a brave tale of survival, loss, and conviction.",
	PullHanselAndGretel1 = "For your evenings entertainment, comes the lost fable of two children attempting to find their way!",
	PullHanselAndGretel2 = "We're sorry grandmother. Your time has expired.",
	PullEngineers = "For your evening viewing we have a tragic story of a humble technician who has been outcast!",
}

-- Dark Rider
L = DBM:GetModLocalization("DarkRider")

L:SetGeneralLocalization{
	name = "Cavalgante Negro"
}

L:SetOptionLocalization{
	EnableMinorNameplates = "Ativar automaticamente as placas de nome inimigas para unidades menores em Karazhan para mostrar Imagens Espelhadas"
}

L:SetMiscLocalization{
	MirrorImage = "Imagem Espelhada",
	EnabledNameplates = "DBM ativou automaticamente as placas de nome para unidades inimigas menores em Karazhan, isso ajuda a detectar imagens espelhadas. Você pode desativar isso nas opções do módulo Cavalgante Negro em Masmorras/Criptas de Karazhan."
}

-- Creeping Malison
L = DBM:GetModLocalization("CreepingMalison")

L:SetGeneralLocalization{
	name = "Praga Sorrateira"
}

-- Karazhan Crypts Trash
L = DBM:GetModLocalization("KCTrash")

L:SetGeneralLocalization{
	name = "Criptas de Karazhan: Lixo"
}

-- Unk'omon
L = DBM:GetModLocalization("Unkomon")

L:SetGeneralLocalization{
	name = "Ink'omum"
}

L:SetWarningLocalization{
	WarnDoom = "Ruína - Remover maldição"
}

L:SetOptionLocalization{
	WarnDoom = "Exibir anúncio para remover $spell:1221577"
}

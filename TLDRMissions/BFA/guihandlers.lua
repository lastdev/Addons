...
gui.CalculateButton:SetScript("OnClick", function (self)
    ...
    for i = 1, 12 do
        ...
        if db.profile.selectedRewards[i] == "gold" then
            ...
            acCategory = "Gold"
        elseif db.profile.selectedRewards[i] == "BFA-resources" then
            newMissions = gui:GetTableResourcesMissions()
            acCategory = "BFAResources"
        ...
        elseif db.profile.selectedRewards[i] == "artifact-power" then
    ...
    calculateNextMission = function()
        ...
        local lineup, result = addon.BFAGUI:arrangeFollowerTroopCombinations(followerLineup, troopLineup, nextMission.missionID, "lowestLevel")
...
RunNextFrame(function()
    hooksecurefunc(addon, "garrisonMissionStartedHandler", function(self, garrFollowerTypeID, missionID)
        if garrFollowerTypeID ~= gui.followerTypeID then return end
...
gui.StartMissionButton:SetScript("OnClick", function(self)
    ...
    local currencyInfo = C_CurrencyInfo.GetCurrencyInfo(addon.BFAGUI.RESOURCE_CURRENCY_ID)

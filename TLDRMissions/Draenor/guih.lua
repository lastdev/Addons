    ...
    for i, button in pairs(gui.checkButtons) do
        for k, v in pairs(db.profile.selectedRewards) do
            if v == rewardStrings[i] then
                ...
                gui.priorityLabels[i]:SetText(i)
...
local function processResults(results)
    ...
    if results.successChance and (results.successChance >= 100) then
        ...
        gui.FailedCalcLabel:SetText()
        gui.CostLabel:Show()
        gui.CostResultLabel:SetText(((C_Garrison.GetMissionCost(missionWaitingUserAcceptance.missionID) or 10)).." "..POWER_TYPE_ANIMA)
    ...
    if db.profile.anythingForXP then
        ...
        for key, mission in pairs(newMissions) do
            if mission.xp <= 500 then
                if not db.profile.anythingForXPCategories["1-500"] then
                    newMissions[key] = nil
                end
            elseif mission.xp <= 600 then
                if not db.profile.anythingForXPCategories["501-600"] then
                    newMissions[key] = nil
                end
            elseif mission.xp <= 700 then
                if not db.profile.anythingForXPCategories["601-700"] then
                    newMissions[key] = nil
                end
            elseif mission.xp <= 800 then
                if not db.profile.anythingForXPCategories["701-800"] then
                    newMissions[key] = nil
                end
            elseif mission.xp <= 900 then
                if not db.profile.anythingForXPCategories["801-900"] then
                    newMissions[key] = nil
                end
            elseif mission.xp <= 1000 then
                if not db.profile.anythingForXPCategories["901-1000"] then
                    newMissions[key] = nil
                end
            else
                if not db.profile.anythingForXPCategories["1000+"] then
                    newMissions[key] = nil
                end
            end
        end
        ...
        local function processing()
            ...
            if useSpecialTreatment then
                if (db.profile.followerXPSpecialTreatmentAlgorithm == 1) or (db.profile.followerXPSpecialTreatmentAlgorithm == 2) then -- "All low level followers, lowest level first"
                    for _, follower in ipairs(followers) do
                        if (not follower.status) and (not alreadyUsedFollowers[follower.followerID]) and (follower.level < 40) then
                            table.insert(followerLineup, follower.followerID)
                        end
                    end
                    ...
                    if numFollowers > 0 then
                        ...
                        for i = 1, median do
                            if (not followers[i].status) and (not alreadyUsedFollowers[followers[i].followerID]) and (followers[i].level < 40) then

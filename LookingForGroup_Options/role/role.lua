...
LookingForGroup_Options:push("role",
{
	...
	args =
	{
		...
		edit_comment =
		{
			...
			type = "execute",
			func = LookingForGroup_Options.signup_empty
		},
		reset =
		{
			order = get_order(),
		...
		{
			order = get_order(),
			name = "|T337497:16:16:0:0:64:64:0:19:1:20|t"..LEADER,
			desc = GUIDE_TOOLTIP,
			type = "toggle",
			get = function(info)
				return GetLFGRoles()
		...
		{
			order = get_order(),
			name = "|T337497:16:16:0:0:64:64:0:19:22:41|t"..TANK,
		...
		{
			order = get_order(),
			name = "|T337497:16:16:0:0:64:64:20:39:1:20|t"..HEALER,
		...
		{
			order = get_order(),
			name = "|T337497:16:16:0:0:64:64:20:39:22:41|t"..DAMAGER,
...
LookingForGroup_Options.Register("auto_accept","f",function(infos,bts,first,profile)
...
LookingForGroup_Options.Register("auto_accept","f",function(infos,bts,first,profile)
	...
	if numclasses <= maxPlayers then
		local this_group = GetGroupMemberCounts()
		local value = 0
	...
	for i=1,#infos do
		...
		if info and bit.band(bts[i],1) == 0 then
			for memberIdx=1,info.numMembers do
				local name, class = C_LFGList.GetApplicantMemberInfo(info.applicantID,memberIdx)
				if counts[class] == 0 then
					counts[class] = 1
				else
					bts[i] = bit.bor(bts[i],1)
